...
-- ------------------------------------
-- 5. 삭제를 적용한 foreign key 제약조건(외래키 제약조건)
...
-- 부모 테이블에서 우수회원등급 삭제
SELECT * FROM user_grade;
DELETE FROM user_grade WHERE grade_code = 20; -- user _foreignkey1 테이블은 삭제룰을적용하지 않고 20을 참조한 데이터가 있다.

DELETE FROM user_foreignkey1 WHERE 1 = 1; -- user_foreignkey1 테이블의 참조하는 데이터 삭제

SELECT * FROM user_foreignkey2;
DELETE FROM user_grade WHERE grade_code = 20; --user_foreignkey1 테이블을 삭제룰 
...
-- null 값으로 변화된 자식 테이블ㅇ의 행들을 새로 추가한 회원 등급으로 업데이트ㅡ 
...
-- default로 insert 또는 update를 하면 defaulte로 세팅된 값 적용
INSERT 
  INTO tbl_country
VALUES (null, default, default, default, default);


-- insert 시 default 설정된 칼럼을 무시하고 insert 하면 null이 아닌 default로 초기 세팅된 값이 들어간다.
INSERT
  INTO tbl_country
(country_code)
VALUES
